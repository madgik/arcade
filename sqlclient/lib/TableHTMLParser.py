...
class TableHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "table":
            ...
            tag == "td"
        ):  #### an exei colspan simeiwse to kai sto telos vale sti litsa ... an exei rowspan to idio an exei kai ta duo
            self.columnIndex += 1
            if "colspan" in dattrs:
                self.replicatecolumn = int(dattrs["colspan"])
            else:
    ...
    def handle_endtag(self, tag):
        if tag == "table":
            ...
            if self.bInspecting == False:
                ...
                while (
                    self.rowIndex in self.rowRepeats
                    and self.columnIndex in self.rowRepeats[self.rowIndex]
                ):
                    self.header += [self.rowRepeats[self.rowIndex][self.columnIndex]]
                    self.columnIndex += 1

                if self.replicatecolumn or self.replicaterow:
                    if self.replicatecolumn:
                        self.replicatecolumn -= 1
                    if self.replicaterow:
                        self.replicaterow -= 1
            ...
            tag == "td"
        ):  ### koitame an proigountai kapoia kopy ta vazoume kai colindex++
            if self.bInspecting == False:
                while (
                    self.rowIndex in self.rowRepeats
                    and self.columnIndex in self.rowRepeats[self.rowIndex]
