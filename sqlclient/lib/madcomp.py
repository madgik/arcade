                    ...
                    for c in range(colnum):
                        ...
                        if len(s) > 1 and ind[c * 2 + 1] == 0 and ind[colnum * 2] > 1:
                            ...
                            if len(s) == 1:
                                ...
                                cols[c] = map(
                                    s.__getitem__,
                                    array("B", decompress(input.read(ind[c * 2 + 1]))),
                                )
                            else:
                                cols[c] = map(
                                    s.__getitem__,
                                    array("H", decompress(input.read(ind[c * 2 + 1]))),
                                )
                ...
                if lens > 50 * 1.0 * lencols / 100:
                    ...
                    if lens > 1:
                        if lens < 256:
                            output.write(
                                self.compress(
                                    array("B", [coldict[y] for y in col]).tostring(),
                                    self.level,
                                )
                            )
                        else:
                            output.write(
                                self.compress(
                                    array("H", [coldict[y] for y in col]).tostring(),
                                    self.level,
                                )
                            )
