...
def schemastr(tablename, colnames, typenames=None):
    stripedcolnames = [
        '"' + el + '"'
        if onlyalphnum.match(el)
        else '"'
        + reduce_spaces.sub(" ", el.replace("\n", "").replace("\t", ""))
        .strip()
        .replace('"', '""')
        + '"'
    ...
    if not typenames:
        return "create table %s(%s)" % (
            tablename,
            ",".join([c for c in stripedcolnames]),
        )
    else:
        stripedtypenames = [
            ""
            if el.lower() == "none" or el == ""
            else el
            if onlyalphnum.match(el)
            else '"' + el.replace('"', '""') + '"'
            for el in typenames
        ...
        return "create table %s(%s)" % (
            tablename,
            ",".join(
                [c + " " + str(t) for c, t in zip(stripedcolnames, stripedtypenames)]
            ),
        )
