...
def query(tree, constraints=None, consargs=None):
    ...
    while direct or len(to_visit) != 0:
        ...
        if leftindex > treedatalen or (
            leftindex >= numberofdata
            and leftindex < treedatalen
            and treedata[leftindex] == None
        ):
            passes = True
            for i in range(k):
...
def intersectranges(range1, range2):
    if (
        EMPTYRANGE in range1
        or EMPTYRANGE in range2
        or range1[0] >= range1[1]
        or range2[0] >= range2[1]
    ):
        return EMPTYRANGE
...
if __name__ == "__main__":
    ...
    print(
        intersectranges(
            [(NORMAL, 5, LEFT), INFPOS], [(NORMAL, 2, RIGHT), (NORMAL, 7, RIGHT)]
        )
    )
    print(
        intersectranges(
            [INFNEG, (NORMAL, 5, LEFT)], [(NORMAL, 2, RIGHT), (NORMAL, 7, RIGHT)]
        )
    )
    print(intersectranges([INFNEG, (NORMAL, 5, RIGHT)], [(NORMAL, 5, LEFT), INFPOS]))
    print(
        intersectranges([INFNEG, (NORMAL, "a", RIGHT)], [(NORMAL, "a", LEFT), INFPOS])
    )
    print(
        intersectranges([INFNEG, (NORMAL, "a", LEFT)], [(NORMAL, "a", RIGHT), INFPOS])
    )
