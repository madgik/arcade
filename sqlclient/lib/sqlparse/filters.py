    ...
    def _process(self, tlist):
        ...
        while grouping.Comment in clss:
            ...
            if (
                ...
                and not prev.is_whitespace()
                and not next_.is_whitespace()
                and not (
                    prev.match(T.Punctuation, "(") or next_.match(T.Punctuation, ")")
                )
            ):
...
class ReindentFilter(Filter):
    ...
    def _split_kwds(self, tlist):
        ...
        idx = 0
        token = tlist.token_next_match(idx, T.Keyword, split_words, regex=True)
        while token:
            ...
            token = tlist.token_next_match(
                tlist.token_index(nl) + offset, T.Keyword, split_words, regex=True
            )
    ...
    def _split_statements(self, tlist):
        ...
        while token:
            ...
            if prev:
                nl = self.nl()
                tlist.insert_before(token, nl)
            token = tlist.token_next_by_type(
                tlist.token_index(token) + 1, (T.Keyword.DDL, T.Keyword.DML)
            )
    ...
    def _process_parenthesis(self, tlist):
        ...
        if first and first.ttype in (T.Keyword.DML, T.Keyword.DDL):
            ...
            indented = True
        num_offset = self._get_offset(tlist.token_next_match(0, T.Punctuation, "("))
