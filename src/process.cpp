...
int Processing::get_column_value(FILE *f1, char ***&cols, int blocknum, long int blockstart, struct fileH fileheader1,
                                 struct D header1, vector<int> columns, int *rowids, int rowidsnum, int join1,
                                 long *data, vector<unordered_map<int, vector<string> *>> &dict_cache,
                                 Caches &mcaches) {
    int colnum = -1;
    int initstep;
    int temp = 0;
    ...
    for (int i: columns) {
        ...
        if (i == join1) {
            ...
            if (header1.dictsize == 0) { // case no dictionary
                ...
                mcaches.get_values(f1, values1, initstep + sizeof(struct D) + header1.previndices * 2,
                                   header1.indicessize);;
                int c = 0;
                ...
                long int seek_position =
                        initstep1 + sizeof(struct D) + header1.dictsize + header1.previndices * 2 + header1.minmaxsize;

                if (header1.bytes == 1) { // two byte offsets /*read offsets of file 1*
                    unsigned short *offsets;
                    int res = mcaches.get_short_offsets(f1, seek_position, header1.indicessize, header1.numofvals,
                                                        offsets);
                    for (int j = 0; j < rowidsnum; j++)
                ...
                if (header1.bytes == 0) { // four byte offsets
                    ...
                    int res = mcaches.get_int_offsets(f1, seek_position, header1.indicessize, header1.numofvals,
                                                      offsets);
                    for (int j = 0; j < rowidsnum; j++)
                        off[j] = offsets[rowids[j]];
                }
                ...
                if (header1.bytes == 2) { // one byte offsets
                    ...
                    int res = mcaches.get_char_offsets(f1, seek_position, header1.indicessize, header1.numofvals,
                                                       offsets);
                    for (int j = 0; j < rowidsnum; j++)
                        off[j] = offsets[rowids[j]];
                }
                ...
                for (int j = 0; j < rowidsnum; j++) {
                    ...
                    if (blocknum != rightblock) {
                        if (dict_cache[i].find(rightblock) == dict_cache[i].end()) {
                            ...
                            mcaches.get_values(f1, dict_cache[i][rightblock],
                                               initstep2 + sizeof(struct D) + header2.minmaxsize +
                                               header2.previndices * 2, header2.dictsize);
                        }
                        ...
                        if (dict_cache[i].find(rightblock) == dict_cache[i].end())
                            mcaches.get_values(f1, dict_cache[i][rightblock],
                                               initstep1 + sizeof(struct D) + header1.minmaxsize +
                                               header1.previndices * 2, header1.dictsize);
                        cols[colnum][c] = &(*dict_cache[i][rightblock])[position_in_block][0];
                        c++;
                    }
                ...
                mcaches.get_values(f1, dict_cache[i][0],
                                   initstep1 + sizeof(struct D) + header1.minmaxsize + header1.previndices * 2,
                                   header1.dictsize);
                for (int j = 0; j < rowidsnum; j++) {
...
int Processing::filter_page(FILE *f1, char ***&cols, int &blocknum, unsigned long &initstep1, int join1,
                            struct fileH fileheader1, int &totalcount1, string &value, vector<int> retcolumns,
                            vector<unordered_map<int, vector<string> *>> &dict_cache, int &global_len, int &offset,
                            long *(&data),
                            Caches &mcaches) {
    ...
    if (header1.dictsize == 0) {
        ...
        if (retcs > 0)
            get_column_value(f1, cols, blocknum, blockstart, fileheader1, header1, retcolumns, rowids, found_index,
                             join1, data, dict_cache, mcaches);
        ...
        if (offset == -1 or header1.diff == 1) {
            ...
            if (header1.lendiff > 0) {
                if (header1.diff == 0) {
                    ...
                    mcaches.get_values(f1, values1,
                                       initstep1 + sizeof(struct D) + header1.previndices * 2 + header1.minmaxsize,
                                       header1.dictsize);
        ...
        if (header1.indicessize == 4) {
            ...
            int offf;
            fseek(f1, initstep1 + sizeof(struct D) + header1.previndices * 2 + header1.dictsize + header1.minmaxsize,
                  SEEK_SET);
            ...
            if (offf == offset) {
                ...
                if (retcs > 0)
                    get_column_value(f1, cols, blocknum, blockstart, fileheader1, header1, retcolumns, rowids,
                                     found_index, join1, data, dict_cache, mcaches);
...
int Processing::scan_page(FILE *f1, char ***&cols, int &blocknum, unsigned long &initstep1, struct fileH fileheader1,
                          int &blockn, int &totalcount1, vector<int> retcolumns, vector<vector<string>> &globaldict,
                          int &global_len,
                          Caches &mcaches) {
    struct D header1;
    ...
    for (int colnum = 0; colnum < columnslen; colnum++) {
        ...
        if (header1.dictsize == 0) { //no dict column
            vector<string> *values1;
            mcaches.get_values(f1, values1,
                               blockposition + current + sizeof(int) * (fileheader1.numofcols + 1) + sizeof(struct D),
                               header1.indicessize);
            int ind = 0;
            for (string &i : *values1)
                cols[colnum][ind++] = &i[0];
            ...
            vector<string> *values1;
            if (header1.lendiff > 0) {
                mcaches.get_values(f1, values1, blockposition + current + sizeof(int) * (fileheader1.numofcols + 1) +
                                                sizeof(struct D) + header1.previndices * 2 + header1.minmaxsize,
                                   header1.dictsize);
                globaldict[colnum].insert(globaldict[colnum].end(), (*values1).begin(), (*values1).end());
            }
            if (header1.indicessize == 4) {
                int unique_offset;
                fseek(f1, blockposition + current + sizeof(int) * (fileheader1.numofcols + 1) + sizeof(struct D) +
                          header1.dictsize + header1.previndices * 2 + header1.minmaxsize, SEEK_SET);
                ...
                long int position =
                        blockposition + current + sizeof(int) * (fileheader1.numofcols + 1) + sizeof(struct D) +
                        header1.dictsize + header1.previndices * 2 + header1.minmaxsize;
                if (header1.bytes == 1) { // two byte offsets /*read offsets of file 1*/
                    unsigned short *offsets;
