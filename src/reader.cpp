...
Generator<int>
ArcadeReader::equi_filter(char *filename, char ***&cols, int col_num, char *val, int *retcols, int colnum) {
    ...
    if (strncmp(marker, "DIFF", 4) != 0) {
        ...
        while (totalcount1 < fileheader1.numofvals) {
            rows = processing.filter_page(f1, cols, blocknum, initstep1, join1, fileheader1, totalcount1, value,
                                          retcolumns, dict_cache, global_len, offset, data, this->mcaches);
            co_yield rows;
        }
        delete[] data;
...
Generator<int>
ArcadeReader::random_access(char *filename, char ***&cols, int *retcols, int colnum, int *rowids, int rowidsnum) {
    cols = (char ***) malloc(colnum * sizeof(char **));
    for (int mal = 0; mal < colnum; mal++) cols[mal] = (char **) malloc(65535 * sizeof(char *));
    ...
    for (int i = 0; i < rowidsnum; i++) {
        ...
        blocknum = (rowids[i]) / (int)data[fileheader1.numofblocks];
        if (blocknum != temp_blocknum and temp_blocknum != -1) {
            rowids[i] = (rowids[i]) % (int)data[fileheader1.numofblocks];
            dict_cache.clear();
            co_yield processing.get_column_value(f1, cols, temp_blocknum, initstep1, fileheader1, header1, columns,
                                                 rowids + start_of_block, rows_per_block, -1, data, dict_cache,
                                                 this->mcaches);
        ...
        if (i == rowidsnum - 1) {
            dict_cache.clear();
            co_yield processing.get_column_value(f1, cols, blocknum, initstep1, fileheader1, header1, columns,
                                                 rowids + start_of_block, rows_per_block, -1, data, dict_cache,
                                                 this->mcaches);
        }
    }
...
Generator<int> ArcadeReader::scan(char *filename, char ***&cols, int *retcols, int colnum) {
    ...
    if (strncmp(marker, "DIFF", 4) != 0)
        ...
        while (totalcount1 < fileheader1.numofvals)
            co_yield processing.scan_page(f1, cols, blocknum, initstep1, fileheader1, blockn, totalcount1, retcolumns,
                                          globaldict, global_len, this->mcaches);
