...
def read_data_page(f,global_dictionary, filters, helper, header, metadata, skip_nulls=False,
                   selfmade=False):
    ...
    if daph.encoding == parquet_thrift.Encoding.PLAIN:
        ...
        values = encoding.read_plain(raw_bytes[io_obj.loc:],global_dictionary, filters,
                                     metadata,
...
def read_dictionary_page(file_obj, schema_helper, page_header, column_metadata):
    ...
    if column_metadata.type == parquet_thrift.Type.BYTE_ARRAY:
        ...
        values = encoding.read_plain(
                raw_bytes,global_dictionary, filters, column_metadata,
                page_header.dictionary_page_header.num_values, width)
    return values


def read_col(column, global_dictionary, filters, schema_helper, infile, use_cat=False,
    ...
    while True:
        ...
        if (selfmade and hasattr(cmd, 'statistics') and
            ...
            skip_nulls = False
        defi, rep, val = read_data_page(infile,global_dictionary, filters, schema_helper, ph, cmd,
...
def read_row_group_file(fn, global_dictionary, filters, rg, columns, categories, schema_helper, cats,
                        open=open, selfmade=False, index=None, assign=None,
                        scheme='hive'):
    with open(fn, mode='rb') as f:
        return read_row_group(f,global_dictionary, filters, rg, columns, categories, schema_helper, cats,
                              selfmade=selfmade, index=index, assign=assign,
                              scheme=scheme)


def read_row_group_arrays(file, global_dictionary, filters, rg, columns, categories, schema_helper, cats,
    ...
    for i,column in enumerate(rg.columns):
        ...
        read_col(column,global_dictionary[i], filters, schema_helper, file, use_cat=name+'-catdef' in out,
...
def read_row_group(file, global_dictionary, filters, rg, columns, categories, schema_helper, cats,
    ...
    read_row_group_arrays(file,global_dictionary, filters, rg, columns, categories, schema_helper,
