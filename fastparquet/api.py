...
class ParquetFile(object):
    ...
    def read_row_group_file(self,  rg, global_dictionary,filters, columns, categories, index=None,
                            assign=None):
        ...
        core.read_row_group_file(
                fn, global_dictionary,filters, rg, columns, categories, self.schema, self.cats,
                open=self.open, selfmade=self.selfmade, index=index,
                assign=assign, scheme=self.file_scheme)
        if ret:
            return df

    def read_row_group(self, rg, global_dictionary,filters, columns, categories, infile=None,
        ...
        if assign is None:
            ...
            ret = True
        core.read_row_group(
                infile,global_dictionary,filters,rg, columns, categories, self.schema, self.cats,
    ...
    def iter_row_groups(self, columns=None, categories=None, filters=[],
        ...
        rgs = self.filter_row_groups(filters[0])
        if all(column.file_path is None for rg in self.row_groups
            ...
            global_dictionary = []
            for i in xrange(len(rg.columns)):
                global_dictionary.append([[],-1,0])

            
            for rg in rs:
                df, views = self.pre_allocate(rg.num_rows, columns,
                                              categories, index)
                self.read_row_group_file(rg, global_dictionary, filters, columns, categories, index,
                                         assign=views)
                yield df
    ...
    def to_pandas(self, columns=None, categories=None, filters=[[],[]], 
        ...
        if self.file_scheme == 'simple':
            with self.open(self.fn, 'rb') as f:
                global_dictionary = []
                for i in xrange(len(rgs[0].columns)):
                    global_dictionary.append([[],-1,0])
                ...
                for rg in rgs:
                    ...
                    self.read_row_group(rg,global_dictionary,filters, columns, categories, infile=f,
        ...
        for filter in filters[0]:
            ...
            df = df[df[filter[0]].notnull()]
            
            
        return df 
            
        #for filter in filters:
         #   df = df[df[filter[0]] == filter[2]]
        ...
        #if filters != []:    
        #    #df.dropna(subset=[x[0] for x in filters],inplace=True)
    ...
    for column in rg.columns:
        ...
        for op, val in app_filters:
            ...
            if column.meta_data.statistics is not None:
                ...
                try:
                    if filter_val(op, float(val), float(s.min), float(s.max)):
                        return True
                except:
                    if filter_val(op, val, s.min, s.max):
                        return True
...
def filter_out_stats1(rg, filters, schema):
    """
    According to the filters, should this row-group be excluded
    Considers the statistics included in the metadata of this row-group
    Parameters
